...
]).then(function(all){var elev=all[0],inflow=all[1],out=all[2],e=last(elev),i=last(inflow),o=last(out);if(!e)throw Error('No elevation');var pool=Number(d.normal_pool_ft),delta=isFinite(pool)?e[1]-pool:null,p=prior24(elev,e[0]),chg=p?e[1]-p[1]:null,release=o?Number(o[1]):null;var status=release==null?'Unknown':release>1?'RELEASING WATER':'NO MEASURABLE RELEASE';var g='';if(d.dam_name)g+=stat('DAM',d.dam_name);g+=stat('RELEASE STATUS',status,release>1?'good':'');g+=stat('CURRENT LAKE LEVEL',e[1].toFixed(2)+' ft','good');if(delta!=null)g+=stat('VS NORMAL POOL',(delta>=0?'+':'')+delta.toFixed(2)+' ft');if(chg!=null)g+=stat('24-HOUR LEVEL CHANGE',(chg>=0?'↑ ':'↓ ')+Math.abs(chg).toFixed(2)+' ft');g+=stat('OUTFLOW / RELEASE',release==null?'—':Math.round(release).toLocaleString()+' cfs');if(i)g+=stat('INFLOW',Math.round(i[1]).toLocaleString()+' cfs');g+=stat('GATE / SPILLWAY STATUS','Not separately published');el('damOpsGrid').innerHTML=g;el('damOpsNote').textContent='USACE Tulsa District CWMS • Updated '+new Date(e[0]).toLocaleString()+'. “Releasing water” is based on measured reservoir outflow. Gate/spillway position is not inferred.'}).catch(function(){el('damOpsGrid').innerHTML=(d.dam_name?stat('DAM',d.dam_name):'')+stat('LIVE DAM DATA','Temporarily unavailable');el('damOpsNote').textContent='No stale lake level or release value is substituted. Use the official water source below.'})}
function boot(){var id=lakeId();if(!id)return;fetchJSON(RAW+'data/lake-intel.json?v=damops1').then(function(j){var d=(j.lakes||{})[id]||{};renderStatic(d,id);renderUSACE(id,d)}).catch(function(){ensureCard();el('damOpsGrid').innerHTML=stat('DAM DATA','Temporarily unavailable')})}
