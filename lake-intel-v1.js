...
function depthSection(id){
  const cfg=bathy&&bathy.lakes&&bathy.lakes[id];if(!cfg)return'';
  if(cfg.status==='available')return '<div class="intelSection"><b>Map data</b><p class="intelDepthGood">〰 Verified OWRB lake-bottom contours available'+(cfg.survey?' • '+esc(cfg.survey):'')+'. Fishing reference only — not for navigation.</p></div>';
...
function render(id){
  ...
  if(!d){box.innerHTML='<h3>Lake Intelligence</h3>'+depthSection(id)+'<p style="font-size:9px;color:#aaa5b2;line-height:1.45;margin:8px 0 0">Detailed fisherman data is being verified for this lake. PittCo will only publish lake specifications and operational data when an authoritative source is confirmed.</p>';return}
  const stats=[stat('SURFACE AREA',fmt(d.surface_acres)+' ac'),stat('SHORELINE',fmt(d.shoreline_miles,1)+' mi'),stat('MAX DEPTH',d.max_depth_ft!=null?fmt(d.max_depth_ft,1)+' ft':null),stat('NORMAL POOL',d.normal_pool_ft!=null?fmt(d.normal_pool_ft,2)+' ft':null),stat('LAKE LENGTH',d.lake_length_miles!=null?fmt(d.lake_length_miles,1)+' mi':null),stat('DAM HEIGHT',d.dam_height_ft!=null?fmt(d.dam_height_ft,0)+' ft':null),stat('VOLUME / STORAGE',d.volume_acre_ft!=null?fmt(d.volume_acre_ft)+' ac-ft':(d.storage_acre_ft!=null?fmt(d.storage_acre_ft)+' ac-ft':null))].join('');
  box.innerHTML='<h3>Lake Intelligence</h3><div class="intelGrid">'+stats+'</div>'+
    ...
    (d.dam_name?'<div class="intelSection"><b>Dam</b><p>'+esc(d.dam_name)+'</p></div>':'')+
...
async function init(){
  ensureStyles();await Promise.all([loadIntel(),loadBathy()]);
